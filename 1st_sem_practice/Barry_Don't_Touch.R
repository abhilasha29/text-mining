...
#build rules
trainegs = read.transactions(file="/Users/abhilashakumari/Downloads/simplebasket.csv",rm.duplicates=TRUE, format="single", sep=",", cols=c("basketID","item"));
...
#read the test data
testegs = read.csv(file="/Users/abhilashakumari/Downloads/simplebasket-test.csv");
...
userpreds = as.data.frame(aggregate(preds ~ basketID, data = testegs, paste, collapse=","))# aggregate works like group by
...
baskets = as.data.frame(aggregate(items ~ basketID, data = testegs, paste, collapse=",")) #aggregating items in basket wrt basket id
baskets$items = apply(baskets,1,function(X) uniqueitems(X["items"])) #finding unique items in basket
