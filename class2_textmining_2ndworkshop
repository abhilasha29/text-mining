...
len(reuters.fileids()) #
cats = [ reuters.categories(f) for f in reuters.fileids() ] # for every file in retuers file reuter list  show categories for the articles
...
cats_flat = [ c for l in cats for c in l ] # contains list of list so flattens each list inside the list
...
grain_tok = [ reuters.words(f) for f in grain ]  # retrieving all token of grain category
